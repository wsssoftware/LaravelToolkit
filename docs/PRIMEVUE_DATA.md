...
- You must create filter data prop to use the filters function. Remember to pass it on DataTableAdapter and DataTable.

Available adapter props:
- **filters**: Here you will pass your filters like you pass to DataTable.
- **filterDebounceWait**: How many milliseconds filter will debounce before do it.
- **globalFilterName**: Here you configure if needed the global filter key. On this filter, it will search all (or configured in Laravel
        ...
        <DataTable
            ...
            <Column field="id" :sortable="true" header="Id">
                <template #filter="{filterModel, filterCallback}">
                    <input v-model="filterModel.value" type="text" @input="filterCallback()" class="p-column-filter" placeholder="Search by country" />
                </template>
            </Column>
...
> - **loading**: You must pass to `sort` prop to handle loading fallback.
